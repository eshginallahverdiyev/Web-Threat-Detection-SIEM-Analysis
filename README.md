...
### 4.4 SQL Injection Attacks — SQLMap

SQL injection attempts were performed against a login form endpoint using SQLMap.

```bash
sqlmap -u "http://<target-ip>/login.php?id=1" --dbs --batch
sqlmap -u "http://<target-ip>/login.php?id=1" -D webapp --tables
sqlmap -u "http://<target-ip>/login.php?id=1" -D webapp -T users --dump
```

![SQL Injection 1](images/kali-sqlinjection.png)

*Figure 4.4 — SQLMap identifying the target as injectable (MySQL backend detected).*

![SQL Injection 2](images/kali-sqlinjection2.png)

*Figure 4.5 — SQLMap enumerating available databases.*

![SQL Injection 3](images/kali-sqlinjection3.png)

*Figure 4.6 — Listing tables within the target database.*

![SQL Injection 4](images/kali-sqlinjection4.png)

*Figure 4.7 — Extracting column names from the `users` table.*

![SQL Injection 5](images/kali-sqlinjection5.png)

*Figure 4.8 — Dumping user credentials (usernames and hashed passwords).*

![SQL Injection 6](images/kali-sqlinjection6.png)

*Figure 4.9 — Final dump complete. Credentials successfully extracted, demonstrating full database compromise.*
...
| `sqlmap/1.x.x-stable` | SQLMap | SQL injection automation |
| `Nikto/2.1.6` | Nikto | Web vulnerability scanning |
| `Mozilla/5.0 Nmap NSE` | Nmap | Port/service scanning |

**Detection Rule Idea:**
```
http.request.headers.user-agent: (*gobuster* OR *sqlmap* OR *nikto* OR *nmap*)
...
| **Tool: SQLMap** | UA: `sqlmap/1.x`, URI payloads | User-Agent + URL query |
...
| Exploit Public-Facing App | T1190 | SQLMap, Nikto |
| OS Fingerprinting | T1592 | Nmap `-A` flag |
| Credential Dumping via SQLi | T1110 | SQLMap `--dump` |
...
3. **Attack Simulation** — Kali Linux used Nmap, Gobuster, Nikto, and SQLMap to mimic a real threat actor
4. **Detection** — Every attack technique left a clear, detectable fingerprint in Kibana:
   ...
   - Known tool User-Agents (Gobuster, SQLMap, Nikto, Nmap)
   - SQL injection payloads in URI query strings
